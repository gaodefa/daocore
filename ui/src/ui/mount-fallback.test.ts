...
describe("Control UI mount fallback", () => {
  ...
  it("shows the static troubleshooting panel when the app element is never registered", async () => {
    ...
    expect(frameWindow.customElements.get("daocore-app")).toBeUndefined();
  ...
  it("keeps the fallback hidden when the app element registers before the timeout", async () => {
    ...
    if (!frameWindow.customElements.get("daocore-app")) {
      frameWindow.customElements.define("daocore-app", class extends frameWindow.HTMLElement {});
    }
    await frameWindow.customElements.whenDefined("daocore-app");
