...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 DaoCore 配置",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密码存入 DaoCore 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 DaoCore 配置",
    ...
    security: {
      ...
      beta: "DaoCore 是一个爱好项目，仍处于 beta 阶段。请预期会有边缘问题。",
      confirm: "我理解 DaoCore 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 DaoCore。",
      learnMore: "了解更多",
      notMultitenant: "DaoCore 默认不是敌对多租户安全边界。",
      personalAgent: "默认情况下，DaoCore 是个人 agent：一个可信操作者边界。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "DaoCore" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 DaoCore 将你加入允许列表。",
      ...
      separatePhoneLabel: "DaoCore 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 DaoCore 用来向 Chat 发送回复的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "DaoCore"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 DaoCore，并启用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 DaoCore。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 DaoCore。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 DaoCore。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 DaoCore。",
