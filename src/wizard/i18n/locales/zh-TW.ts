...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 DaoCore 設定",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密碼存入 DaoCore 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 DaoCore 設定",
    ...
    security: {
      ...
      beta: "DaoCore 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
      confirm: "我理解 DaoCore 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 DaoCore。",
      learnMore: "了解更多",
      notMultitenant: "DaoCore 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，DaoCore 是個人 agent：一個可信操作者邊界。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "DaoCore" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 DaoCore 將你加入允許清單。",
      ...
      separatePhoneLabel: "DaoCore 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 DaoCore 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "DaoCore"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 DaoCore，並啟用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 DaoCore。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 DaoCore。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 DaoCore。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 DaoCore。",
